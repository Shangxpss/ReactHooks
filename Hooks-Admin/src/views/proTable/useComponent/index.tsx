...
const UseComponent = () => {
	...
	// nodesGroup
	const nodesGroup = useRef<d3.Selection<SVGGElement, unknown, HTMLElement, any>>();

	//linksGroup
	const linksGroup = useRef<d3.Selection<SVGGElement, unknown, HTMLElement, any>>();

	const Root = useRef<Person[]>([]);
	...
	function update() {
		...
		// console.log(rootHierarchy, "rootHierarchy");
		const stratify = rootHierarchy(Root.current);
		// const stratify = d3.hierarchy<Data>(rootData);
		const tree = d3.tree<Person>().nodeSize([10, clientWidth / (1 + stratify.height)]);
		const diagonal = d3
			.linkHorizontal()
			.x(d => d.y)
			.y(d => d.x);
		// const
		const nodes = stratify.descendants();
		const links = stratify.links();
		tree(stratify);
		console.log(stratify, "stratify");
		console.log(nodes, "nodes");
		console.log(links, "links");
		console.log(nodesGroup.current, "nodesGroup.current");
		const gNode = nodesGroup.current?.selectAll("g").data(nodes, function (d) {
			// console.log(d, "d");
			return d.id;
		});
	...
	function init() {
		...
		nodesGroup.current = svg.current.append("g").attr("cursor", "pointer").attr("pointer-events", "all");
	}

	// function DrawRoot() {
	// 	if (!svg.current) return;
	// 	const title = svg.current
	// 		.append("g")
	// 		.attr("id", "RootTitle")
	// 		.attr("transform", `translate(${centerPoint[0]}, ${centerPoint[1]})`);

	// 	title.append("rect").attr("width", 100).attr("height", 20).attr("rx", 5).attr("ry", 5).attr("fill", "blue").attr("y", 0);
	// 	title.append("text").text("Root").attr("fill", "black").attr("x", 0).attr("y", 5);
	// }
